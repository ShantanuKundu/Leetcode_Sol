...
Time Complexity  : O(n^2) Used Selection Sort. Can be optimized by using Merge Sort or .sort() function, which will give a
...
public:
    int heightChecker(vector<int>& heights) {
        vector<int> old_heights;
        int n =heights.size(), temp;
        for(int i=0; i<n; i++)
            old_heights.push_back(heights[i]);
